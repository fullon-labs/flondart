...
void main() {
  group('FLON Client', () {
    late FLONClient client;

    setUp(() {
      client = FLONClient('https://1127.0.0.1:8888', 'v1');
    });
    ...
    test('Get Abi', () {
      client.getAbi('flon.token').then((AbiResp abi) {
        expect(abi.accountName, equals('flon.token'));
      });
    });
    ...
    test('Get Account', () {
      client.getAccount('flon.stake').then((Account account) {
        expect(account.accountName, equals('flon.stake'));
